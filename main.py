...
def solicitar_notas():
    ...
    for i in range(1, 5):
        while True:
            try:
                nota = float(input(f"{Fore.YELLOW}Digite a nota do {i}º bimestre (0 a 10): {Style.RESET_ALL}")) # Erro: intervalo de notas deveria ser 0 a 10
                if 0 <= nota <= 10:
                    notas.append(nota)
                    break
                else:
...
def calcular_media(notas):
    ...
    if len(notas) == 0: # Erro: se o vetor estiver vazio, pode gerar divisão por zero
        return 0, 0 
    soma = sum(notas)
...
def exibir_grafico(notas):
    ...
    bimestres = ['1º', '2º', '3º', '4º']  # Erro: bimestre errado, deveria ser '4º' no lugar de '5º'
...
def exibir_resultado(nome, soma, media, notas):
    ...
    if nome in recuperacao:
        ...
        if soma >= 20:  # Erro: soma mínima deveria ser 20, não 25
...
def revisar_notas(notas):
    ...
    for i in range(4):  # Erro: deveria ser 4 bimestres, não 5
        while True:
            try:
